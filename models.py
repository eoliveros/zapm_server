...
#            session.commit()
    ...
    def update_wallet_address(cls, session, user):
        ...
        #print(seeds)
        for seed in seeds:
            #print("Wallet address: "+seed.wallet_address)
            if seed.wallet_address:
                # update txs
                limit = 100
                oldest_txid = None
                txs = []
                while True:
                    have_tx = False
                    txs = blockchain_transactions(logger, app.config["NODE_ADDRESS"], seed.wallet_address, limit, oldest_txid)
                    for tx in txs:
                        oldest_txid = tx["id"]
                        have_tx = MerchantTx.exists(db.session, seed, oldest_txid)
                        if have_tx:
                            break
                        if tx["type"] == 4 and tx["assetId"] == app.config["ASSET_ID"]:
                            amount_nzd = apply_merchant_rate(tx['amount'], seed, app.config, use_fixed_fee=False)
                            date = datetime.datetime.fromtimestamp(tx['timestamp'] / 1000)
                            session.add(MerchantTx(user, date, seed.wallet_address, tx['amount'], amount_nzd, tx['id'], tx['direction'], tx['attachment']))
                    if have_tx or len(txs) < limit:
                        break
                session.commit()
